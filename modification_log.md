...
# 1.查看并熟悉项目
除了学习如何fork他人项目和利用Markdown编写README.md等文件外，还需熟悉CatraMMS此开源项目的各项内容，故向AI提问阅读开源项目的技巧,其中提到需特别阅读README.md、INSTALL.md部分。

# 2.部署项目的前提
向AI提问部署的前提条件，AI提到要创建虚拟环境，其中一个较容易出现的问题是，如果开源项目基于Python,创建venv环境需保证Python版本保持在3.1+。

# 3.下载依赖
不熟悉开源项目的依赖需要下载哪些，为避免依赖过多带来的卡顿问题，向AI提问部署开源项目一般需要安装什么依赖，这里基于Python在数据库依赖上下载MySQL、PostgreSQL、MongoDB等，构建和打包工具依赖选择Gradle，更具体的依赖需要看项目的性质等来变换。

# 4.克隆项目到电脑并配置环境变量
获取项目地址后使用Git克隆，之后的这一部分较为困难，AI回答一般需要数据库连接和秘钥来配置，设置环境变量时可选择.env文件，其余的配置仍需要参考目标项目的文档。

# 5.生产环境部署
这部分内容并不熟悉，AI提示这部分对于开源项目至关重要，关系到项目的稳定性和用户体验。且推荐使用Docker来完成，先通过构建镜像生成可部署的包，再运行容器。

# 6.访问测试
打开浏览器访问测试(Python Web)，并查看运行日志是否有报错。各种报错都可以复制询问AI，一般依赖类和环境配置的报错都可以成功处理。

# 7.问题排查
AI最后提示，在运行整个项目前可以进行几个安全排查避免漏洞，例如端口冲突和权限问题等，其余的问题检查，建议全程参考官方文档。
