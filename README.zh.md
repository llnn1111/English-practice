...
![选择文件](images/选择文件.png)
     2. 系统返回该数据的唯一标识符 。
![返回唯一标识符](images/返回唯一标识符.png)
...
## 二、任务配置阶段  
写个任务单子：
   ![Json代码](images/人脸识别任务Json代码.png)
...
# 1.项目结构分析
## 1.1
代码相关目录：API/src、MMSEngine/src、MMSEngineService/src这些目录存放了项目的核心代码，分别涉及应用程序接口、媒体管理引擎及其服务的实现。
## 1.2
配置目录：conf目录存放配置文件，例如retention for ingestionjobs相关配置，安装部署时需要根据实际需求修改这些配置。
## 1.3
文档相关目录：docs和generateHtmlDoc目录与项目文档有关，docs目录中的文档包含项目的详细说明，在安装部署过程中可作为参考。
## 1.4
脚本目录：scripts目录中的脚本用于辅助安装、启动、停止项目等操作，如IntroOverlay的脚本更新记录，推测脚本在项目运行过程中起到关键作用。prepareToDeploy目录中的脚本也与部署相关。
## 1.5
测试目录：stressTest目录用于压力测试，在部署完成后，可利用该目录中的测试工具对项目进行性能测试。

# 2.安装与部署步骤推测
## 2.1
环境准备：由于项目使用 C++ 编写，需要安装 C++ 开发环境，包括编译器（如 GCC 或 Clang）、构建工具（如 CMake）。同时，还需要安装项目依赖的其他库。
## 2.2
获取项目代码：使用 Git 命令将项目代码克隆到本地。在终端中执行git clone https://github.com/giulianoc/CatraMMS.git，将项目代码下载到本地目录。
## 2.3
处理子模块：项目包含.gitmodules文件，可能存在子模块。执行git submodule init和git submodule update命令初始化并更新子模块，确保项目依赖的所有代码都被获取。
## 2.4
构建项目：进入项目根目录，创建一个构建目录，然后进入该目录。执行cmake..命令，根据项目的CMakeLists.txt文件生成构建脚本，接着执行make命令进行项目构建。
## 2.5
配置项目：修改conf目录中的配置文件。设置摄入作业的保留时间、配置数据库连接信息、设置媒体文件的存储路径等。
## 2.6
部署项目：项目包含服务端和客户端，需要分别部署。对于服务端，需要将构建生成的可执行文件部署到服务器上，并配置服务器环境。对于客户端，需要部署到 Web 服务器上，并确保与服务端的通信正常。
## 2.7
测试项目：使用stressTest目录中的工具对项目进行压力测试，检查项目在高负载情况下的性能和稳定性。同时，进行功能测试，确保项目的摄入、处理、编码、交付等功能正常运行。在实际安装部署过程中，必须参考项目的 Wiki 页面（https://github.com/giulianoc/CatraMMS/wiki ），以获取更准确和详细的安装部署指南。
...
## 5.10 安装 Docker 和 Docker Compose
...
# 测试与验证
访问 http://your_domain_or_ip/admin登录管理界面，验证用户管理、患者录入、预约等功能
